...
from education.education_apps.educa.services import ControlTestResultService
from education.education_apps.educa.selectors import ControlTestResultSelectors, QuestionSelectors


class ControlTestResultDetailApi(APIView):
    ...
    def get(self, request, control_test_result_id):
        control_test_result = ControlTestResultSelectors().get_control_test_result(control_test_result_id)
        if control_test_result is None:
            raise Http404
        data = self.OutputSerializer(control_test_result).data
        return Response(data)


class ControlTestResultCheckInApi(APIView):

    class InputSerializer(serializers.Serializer):
        check_in = serializers.BooleanField()

    def post(self, request, control_test_result_id):
        ...
        if serializer.validated_data['check_in']:
            ...
            if control_test_result is None:
                raise Http404
            control_test_result = ControlTestResultService().control_test_result_update(
                control_test_result=control_test_result, data=serializer.validated_data
            )
            data = ControlTestResultDetailApi.OutputSerializer(control_test_result).data
            return Response(data)
        raise Http404
class ControlTestResultUpdateApi(APIView):
    pass
    # class InputSerializer(serializers.Serializer):
...
class ControlTestResultListApi(APIView):
...
class ControlTestResultCreateApi(APIView):

    class InputSerializer(serializers.Serializer):
        roster = serializers.IntegerField()
        control_test = serializers.IntegerField()
        answers_lst = serializers.ListField()
    def post(self, request):

        serializer = self.InputSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        control_test_result = ControlTestResultService().control_test_result_create(
